...
export class SharingController {
  ...
  @Get('mine') mine(@Req() req: Request) { return this.sharing.listForUser(this.user(req)); }
  @Get(':id/content') mineContent(@Req() req: Request, @Param('id', ParseUUIDPipe) id: string, @Query('resourceType') resourceType?: any, @Query('resourceId') resourceId?: string) { return this.sharing.sharedContent(this.user(req), id, resourceType, resourceId); }
